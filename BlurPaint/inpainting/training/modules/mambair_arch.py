...
class CAB(nn.Module):
    def __init__(self, num_feat, is_light_sr=False, compress_ratio=2, squeeze_factor=30):
...
class SS2D(nn.Module):
    ...
    def forward(self, x: torch.Tensor, **kwargs):
        ...
        x = self.act(self.conv2d(x)) # silu(groups_conv)
...
class VSSModule(nn.Module):
    ...
    def forward(self, input):
        # x=[B, H, W, C]
        B, _, _, C = input.shape
        image_size = input.shape[1:3]
        x = self.ln_1(input)
        x = input*self.skip_scale + self.drop_path(self.self_attention(x))
        x = x * self.skip_scale2 + self.conv_blk(self.ln_2(x).permute(0, 3, 1, 2).contiguous()).permute(0, 2, 3, 1).contiguous()
        x = x.view(B, -1, C).contiguous()
        x = x.view(B, *image_size, C).contiguous()  # [B,H,W,C]
        return x

class SSD_Mamba(nn.Module):
    def __init__(
            self,
            hidden_dim: int = 0,
            drop_path: float = 0,
    ...
    def forward(self, input):
        # x=[B, T, C]
        B, _, C = input.shape
        H = W = int(math.sqrt(input.shape[1]))
        input = input.view(B, H, W, C) # [B,H,W,C]
        # x = self.ln_1(input)
        # x = input*self.skip_scale + self.drop_path(self.self_attention(x))
        x = self.drop_path(self.self_attention(input))
...
class VSSBlock(nn.Module):
    ...
    def forward(self, input, x_size, time_emb=None):
        # x [B,HW,C]
        B, L, C = input.shape
...
class BasicLayer(nn.Module):
    ...
    def forward(self, x, x_size, time):
...
class MambaIR(nn.Module):
    ...
    def __init__(self,
        ...
        self.skip_conv = nn.Conv2d(in_chans, out_chans, kernel_size=3, padding=1)
    ...
    def forward(self, x, time):
        ...
        if self.upsampler == 'pixelshuffle':
            ...
            res = self.conv_after_body(self.forward_features(x_first, time)) + x_first
...
class ResidualGroup(nn.Module):
    ...
    def forward(self, x, x_size, time):
        return self.patch_embed(self.conv(self.patch_unembed(self.residual_group(x, x_size, time), x_size))) + x
